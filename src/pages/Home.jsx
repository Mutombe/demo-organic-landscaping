    ...
    <section ref={containerRef} className="relative min-h-screen bg-white overflow-hidden">
        ...
        <div className="grid lg:grid-cols-2 gap-12 lg:gap-20 items-center min-h-[70vh]">
          <motion.div style={{ y: textY }}>
            <motion.div initial={{ scaleX: 0 }} animate={{ scaleX: 1 }} transition={{ duration: 0.8, delay: 0.2 }}
              className="w-16 h-[3px] bg-green-700 mb-6 origin-left" />
            <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.3 }}
              className="text-green-700 text-xs sm:text-sm font-semibold uppercase tracking-[0.3em] mb-6" style={{ fontFamily: 'var(--font-sans)' }}>
              {hero.badge}
            </motion.p>
            <div className="overflow-hidden">
              {['TRANSFORM', 'YOUR SPACE.'].map((line, i) => (
                <motion.div key={line} initial={{ y: '110%' }} animate={{ y: 0 }}
                  ...
                  <h1 className={`font-heading leading-[0.88] tracking-tight ${
                    i === 1 ? 'text-green-700' : 'text-navy-900'
            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1.2 }} className="flex items-center gap-3 mt-6">
              <div className="flex items-center gap-0.5">
                {[...Array(Math.round(business.rating))].map((_, i) => <Star key={i} size={14} weight="fill" className="text-green-700" />)}
              </div>
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 1.4 }} className="flex flex-wrap gap-4 mt-10">
              <Link to="/contact" className="group inline-flex items-center gap-3 bg-green-700 text-white px-8 py-4 text-sm uppercase tracking-[0.15em] font-semibold transition-all duration-500 hover:bg-green-800 hover:shadow-xl hover:shadow-green-700/20" style={{ fontFamily: 'var(--font-sans)' }}>
                {hero.ctaPrimary} <ArrowRight size={16} className="transition-transform group-hover:translate-x-1" />
              </Link>
              <Link to="/services" className="group inline-flex items-center gap-3 border-2 border-navy-900/10 text-navy-900 px-8 py-4 text-sm uppercase tracking-[0.15em] font-semibold transition-all duration-500 hover:border-green-700 hover:text-green-700" style={{ fontFamily: 'var(--font-sans)' }}>
                {hero.ctaSecondary}
              </Link>
            </motion.div>
          </motion.div>
          <motion.div initial={{ opacity: 0, x: 60 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 1, delay: 0.5 }} className="relative">
            ...
            <div className="absolute -top-3 -left-3 w-20 h-20 border-t-2 border-l-2 border-green-700/30" />
            <div className="absolute -bottom-3 -right-3 w-20 h-20 border-b-2 border-r-2 border-green-700/30" />
            <div className="absolute -bottom-6 -left-6 bg-green-700 text-white p-5 sm:p-6 shadow-2xl">
              <div className="text-center">
                <div className="font-heading text-2xl sm:text-3xl leading-none font-bold">{business.projectsCompleted || '500+'}</div>
                <div className="text-white/70 text-[10px] uppercase tracking-[0.15em] mt-1" style={{ fontFamily: 'var(--font-sans)' }}>Buildies</div>
              </div>
            </div>
          </motion.div>
        </div>
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 2 }}
        ...
        <motion.div animate={{ y: [0, 8, 0] }} transition={{ repeat: Infinity, duration: 2, ease: 'easeInOut' }}>
          <ArrowDown size={14} className="text-green-700/40" />
        </motion.div>
      </motion.div>
    </section>
  );
...
/* ================================================================
   2. PROCESS — Consult > Design > Build
   ================================================================ */
function ProcessSteps() {
  const ref = useRef(null);
  const inView = useInView(ref, { once: true, margin: '-80px' });
  const steps = [
    { num: '01', title: 'Consult', desc: 'Share your vision. We visit your property for a free consultation and site assessment.', icon: Package },
    { num: '02', title: 'Design', desc: 'Our team creates a detailed landscape design with 3D visualizations and plant selections.', icon: Target },
    { num: '03', title: 'Build', desc: 'Watch your outdoor space transform as our skilled crew brings the design to life.', icon: CheckCircle },
  ];
  return (
    <section ref={ref} className="bg-earth-50 py-24 sm:py-32 lg:py-40">
      <div className="max-w-7xl mx-auto px-5 sm:px-8 lg:px-12">
        <motion.div initial={{ opacity: 0, y: 30 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.8 }} className="text-center mb-16 sm:mb-20">
          ...
          <p className="text-green-700/60 text-xs uppercase tracking-[0.3em] mb-3" style={{ fontFamily: 'var(--font-sans)' }}>Our Process</p>
          <h2 className="font-heading text-navy-900 leading-[0.92]" style={{ fontSize: 'clamp(2rem, 4.5vw, 3.5rem)' }}>
            How We <span className="text-green-700">Work</span>
          </h2>
        </motion.div>
        <div className="grid md:grid-cols-3 gap-8 relative">
          <div className="hidden md:block absolute top-16 left-[16%] right-[16%] h-[2px] bg-gradient-to-r from-green-700/20 via-green-700/40 to-green-700/20 z-0" />
          {steps.map((step, i) => (
            <motion.div key={step.title} initial={{ opacity: 0, y: 30 }} animate={inView ? { opacity: 1, y: 0 } : {}}
              ...
              <div className="w-16 h-16 bg-green-700 text-white flex items-center justify-center mx-auto mb-6 shadow-lg shadow-green-700/20">
                <step.icon size={28} weight="fill" />
              </div>
              <div className="text-green-700/20 font-heading text-4xl font-bold mb-2">{step.num}</div>
    ...
    <section ref={ref} className="bg-white py-24 sm:py-32 lg:py-40">
      <div className="max-w-7xl mx-auto px-5 sm:px-8 lg:px-12">
        <motion.div initial={{ opacity: 0, y: 30 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.8 }} className="mb-14 sm:mb-20">
          <div className="w-12 h-[3px] bg-green-700 mb-6" />
          <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-6">
            <div>
              <p className="text-green-700/60 text-xs uppercase tracking-[0.3em] mb-3" style={{ fontFamily: 'var(--font-sans)' }}>What We Create</p>
              <h2 className="font-heading text-navy-900 leading-[0.92]" style={{ fontSize: 'clamp(2rem, 4.5vw, 3.5rem)' }}>
                What We <span className="text-green-700">Build</span>
              </h2>
            </div>
            <Link to="/services" className="group text-navy-900/30 text-xs uppercase tracking-[0.2em] flex items-center gap-2 hover:text-green-700 transition-colors" style={{ fontFamily: 'var(--font-sans)' }}>
            ...
            return (
              <motion.div key={service.title} initial={{ opacity: 0, y: 30 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.7, delay: 0.08 * i }}>
                <Link to={`/services#${services?.items?.[i]?.slug || ''}`} className="group relative block overflow-hidden aspect-[3/4] bg-earth-50">
                  ...
                  <div className="absolute top-5 left-5 z-10 w-10 h-10 bg-green-700 flex items-center justify-center shadow-lg">
                  ...
                  <div className="absolute bottom-0 left-0 right-0 h-[3px] bg-green-700 z-10" />
                </Link>
              </motion.div>
            );
          })}
...
function StatsBand() {
  ...
  return (
    <section ref={ref} className="relative bg-green-700 overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-5 sm:px-8 lg:px-12">
        <div className="grid lg:grid-cols-2 gap-16 lg:gap-24 items-center">
          <motion.div initial={{ opacity: 0, x: -40 }} animate={inView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.9 }}>
            <div className="w-12 h-[3px] bg-green-700 mb-6" />
            <p className="text-green-700/60 text-xs uppercase tracking-[0.3em] mb-3" style={{ fontFamily: 'var(--font-sans)' }}>About Us</p>
            <h2 className="font-heading text-navy-900 leading-[0.95] mb-8" style={{ fontSize: 'clamp(2rem, 5vw, 3.5rem)' }}>
              Harare's<br /><span className="text-green-700">Hottest Courier</span>
            </h2>
            <p className="text-navy-700/50 text-sm sm:text-base leading-relaxed mb-6 max-w-lg" style={{ fontFamily: 'var(--font-sans)' }}>
              Axor Services brings the heat to Harare's delivery scene. We believe every parcel deserves VIP treatment — from a single envelope to a pallet of goods. Our network covers every corner of Harare and extends to all major Zimbabwean cities.
            </p>
            ...
            <div className="flex gap-10 sm:gap-16">
              <div>
                <div className="text-green-700 font-heading text-3xl sm:text-4xl leading-none font-bold">{business.established || '2018'}</div>
                <div className="text-navy-900/30 text-[10px] uppercase tracking-[0.2em] mt-2" style={{ fontFamily: 'var(--font-sans)' }}>Founded</div>
              </div>
              <div>
                <div className="text-green-700 font-heading text-3xl sm:text-4xl leading-none font-bold">{business.projectsCompleted || '500+'}</div>
                <div className="text-navy-900/30 text-[10px] uppercase tracking-[0.2em] mt-2" style={{ fontFamily: 'var(--font-sans)' }}>Buildies</div>
              </div>
              <div>
                <div className="text-green-700 font-heading text-3xl sm:text-4xl leading-none font-bold">{business.rating}</div>
          ...
          <motion.div initial={{ opacity: 0, x: 40 }} animate={inView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.9, delay: 0.2 }} className="relative">
            ...
            <div className="absolute -top-3 -left-3 w-16 h-16 border-t-2 border-l-2 border-green-700/30" />
            <div className="absolute -bottom-3 -right-3 w-16 h-16 border-b-2 border-r-2 border-green-700/30" />
          </motion.div>
        </div>
      </div>
    </section>
...
function WhyChooseUs() {
  ...
  const inView = useInView(ref, { once: true, margin: '-80px' });
  const points = [
    { title: 'Designed Buildies', desc: 'Every parcel tracked in real-time via GPS. Know exactly where your shipment is.' },
      ...
      <div className="max-w-7xl mx-auto px-5 sm:px-8 lg:px-12">
        <motion.div initial={{ opacity: 0, y: 30 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.8 }} className="text-center mb-14 sm:mb-20">
          ...
          <h2 className="font-heading text-navy-900 leading-[0.95]" style={{ fontSize: 'clamp(2rem, 4.5vw, 3rem)' }}>
            Why Choose <span className="text-green-700">Axor</span>
          </h2>
        </motion.div>
        <div className="grid sm:grid-cols-2 gap-6">
          {points.map((point, i) => (
            <motion.div key={point.title} initial={{ opacity: 0, y: 20 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6, delay: 0.1 * i }}
              className="flex gap-5 bg-white p-8 border border-navy-900/5 hover:border-green-700/20 transition-colors duration-500">
    ...
    <section ref={ref} className="relative bg-navy-950 py-24 sm:py-32 lg:py-40 overflow-hidden">
      ...
      <div className="relative z-10 max-w-4xl mx-auto px-5 sm:px-8 lg:px-12">
        <motion.div initial={{ opacity: 0, y: 30 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.8 }} className="text-center">
          <Quotes size={48} weight="fill" className="text-green-600/15 mx-auto mb-8" />
          <AnimatePresence mode="wait">
            <motion.div key={active} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} transition={{ duration: 0.6 }}>
              ...
              <div className="flex flex-col items-center gap-3">
                {t.avatar && <img src={t.avatar} alt={t.name} className="w-12 h-12 rounded-full object-cover border-2 border-green-600/30" loading="lazy" />}
                ...
                <div className="flex items-center gap-0.5 mt-1">
                  {[...Array(t.rating)].map((_, i) => <Star key={i} size={12} weight="fill" className="text-green-600" />)}
                </div>
              </div>
            </motion.div>
          </AnimatePresence>
          <div className="flex items-center justify-center gap-6 mt-12">
            <button onClick={prev} className="w-10 h-10 border border-white/10 flex items-center justify-center text-white/30 hover:text-green-600 hover:border-green-600/30 transition-colors" aria-label="Previous"><CaretLeft size={16} /></button>
            <div className="flex gap-2">
              {homeTestimonials.map((_, i) => <button key={i} onClick={() => setActive(i)} className={`h-[2px] transition-all duration-500 ${i === active ? 'w-10 bg-green-600' : 'w-3 bg-white/10'}`} aria-label={`Testimonial ${i + 1}`} />)}
            </div>
            <button onClick={next} className="w-10 h-10 border border-white/10 flex items-center justify-center text-white/30 hover:text-green-600 hover:border-green-600/30 transition-colors" aria-label="Next"><CaretRight size={16} /></button>
          </div>
        </motion.div>
      </div>
    </section>
    ...
    <section ref={ref} className="relative py-28 sm:py-36 lg:py-48 overflow-hidden">
      ...
      <div className="relative z-20 max-w-5xl mx-auto px-5 sm:px-8 lg:px-12 text-center">
        <motion.div initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: '-80px' }} transition={{ duration: 1 }}>
          <div className="w-16 h-[3px] bg-green-600 mx-auto mb-8" />
          <h2 className="font-heading text-white leading-[0.92] mb-8" style={{ fontSize: 'clamp(2.5rem, 8vw, 6rem)' }}>
            YOUR GARDEN<br /><span className="text-green-600">AWAITS</span>
          </h2>
          <p className="text-white/50 text-sm sm:text-base lg:text-lg max-w-lg mx-auto mb-12 leading-relaxed" style={{ fontFamily: 'var(--font-sans)' }}>
            {homeCta?.subtitle || 'Book a free consultation and let us transform your outdoor space into a living masterpiece.'}
          </p>
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            <Link to="/contact" className="group inline-flex items-center gap-3 bg-green-700 text-white px-8 py-4 text-sm uppercase tracking-[0.15em] font-semibold transition-all duration-500 hover:bg-green-800 hover:shadow-xl hover:shadow-green-700/25" style={{ fontFamily: 'var(--font-sans)' }}>
